...
- Retry：可靠确认原 Session 为 `stopped` 后，复用同一 GovernanceRun、Trigger ID、Session ID、固定输入和成功快照；失败步骤 attempt 增加。PUBLISH Retry 不重新读取来源。Session 在首步骤或下一步骤开始前终止时，从第一个未开始步骤恢复；Retry 控制 Run 在 Runner 重新进入前终止时产生下一 attempt，Runner 重新进入后再次终止也会先复核 Session 终态再产生后续 attempt，且控制面返回的 Session ID 必须与原 Run 一致。
