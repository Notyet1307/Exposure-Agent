...
| --- | --- |
| `needs-triage` | A maintainer still needs to classify or clarify the issue. |
| `needs-info` | Progress requires information unavailable from the repository, tracker, or tools. |
| `wontfix` | The issue will not be actioned. |
| `wayfinder:map` | Historical or contextual map issue; not implementation scope. |
| `wayfinder:research` | Research classification only. |
| `wayfinder:prototype` | Prototype classification only. |
