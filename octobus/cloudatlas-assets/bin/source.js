...
const string = "string";
const common = {
  id: "identity", ip: string, status: string,
...
// E12 overrides only IP bu (declared string, observed id/name object).
const fields = {
  ip: {
    ...common, version: "integer", subnet: "nullableString", live_port: "integer",
    provider: string, as_name: string, as_num: string, location: string,
    country: string, province: string, city: string,
    sources: [{ source: string, reason: string, factor: string, lastseen_at: string }],
  },
  port: {
    ...common, port: "integer", protocol: string, service: string, tunnel: string,
    product: string, version: string, banner: "nullableString", categories: [string],
  },
};
...
function project(value, schema, delta) {
  ...
  for (const [key, type] of Object.entries(schema)) {
    if (Object.hasOwn(value, key)) result[key] = project(value[key], type, delta);
  }
  return result;
}
...
export function normalizePage(text, domain, page, size, spaceId) {
  ...
  const items = data.items.map((item) => {
    if (!object(item) || typeof item.id !== "bigint" || typeof item.ip !== "string" || !isIP(item.ip) || item.ip.includes("%")) fail();
    if (domain === "ip" && item.status !== "valid") fail();
    if (domain === "port" && (!Object.hasOwn(item, "port") || integer(item.port) < 0 || integer(item.port) > 65535)) fail();
    if (Object.hasOwn(item, "bu") && (!object(item.bu) || typeof item.bu.id !== "bigint")) fail();
    if (Array.isArray(item.tags) && item.tags.some((tag) => !object(tag) || typeof tag.pk !== "bigint")) fail();
    return project(item, fields[domain], delta);
  });
...
export async function listPage(ctx, domain) {
  ...
  const url = new URL(domain === "ip" ? "/openapi/v1/asset/ip" : "/openapi/v1/attack/port", base.origin);
  url.search = new URLSearchParams({ space: spaceId, page: String(page), size: String(size), sort: "-id", ...(domain === "ip" ? { status: "valid" } : {}) }).toString();
