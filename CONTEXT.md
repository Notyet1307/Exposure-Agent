...
**Model Connection Revision（模型连接版本）**：
部署级模型连接的一份不可变版本，标识特定端点、协议、模型及凭据版本；新版本不改变旧任务已经选择的模型身份。
_Avoid_: OMP 开发模型、SourceInstance、任意 Provider 路由

**Model Validation（模型验证）**：
针对明确连接版本和固定合成材料记录的验证结果；连接认证、核查能力与项目材料许可是不同结论，验证通过不等于该版本已经启用。
_Avoid_: 通用可用绿灯、客户数据外发许可、启用成功

**Active Model Connection（生效模型连接）**：
当前允许新 AI 任务选择的部署级连接版本；启用另一版本不改写已经固定版本的任务和历史结果。
_Avoid_: 最新保存版本、最近一次验证版本、运行中任务的动态模型

**Task Model Binding（任务模型绑定）**：
一次 AI 任务固定的连接版本、模型及执行身份。后续恢复沿用该绑定，撤销仍可阻断后续调用，不能通过切换当前生效连接替换原任务的模型。
_Avoid_: 当前模型配置、历史结果重算、自动 fallback
